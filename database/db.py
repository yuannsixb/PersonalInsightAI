...
def load_user_rules_cache():
    ...
    返回 (simple_dict, keyword_list):
      simple_dict = {app_name: scene}  — 规则不带关键词，纯进程名匹配
      keyword_list = [(app_name, keyword, scene), ...]  — 规则带关键词，需同时匹配进程名+窗口标题
    """
    conn = get_conn()
    rows = conn.execute(
        'SELECT app_name, window_title_keyword, scene FROM user_scene_dictionary'
    ).fetchall()
    simple = {}
    keyword = []
    for r in rows:
        app = r['app_name'].lower().strip()
        kw = (r['window_title_keyword'] or '').strip()
        scene = r['scene']
        if kw:
            keyword.append((app, kw.lower(), scene))
        else:
            simple[app] = scene
            # 同时存去掉 .exe 的变体
            if app.endswith('.exe'):
...
def match_user_rule(process_name, window_title='', rules_cache=None):
    ...
    新版 cache 格式: (simple_dict, keyword_list)
    """
    ...
    if rules_cache is not None:
        simple, keyword = rules_cache
        # 1. 先查带关键词的规则（进程名+标题同时匹配）
        if title:
    ...
    for sname in search_names:
        # 先查带关键词的
        rows = conn.execute(
            'SELECT scene FROM user_scene_dictionary WHERE app_name = ? AND window_title_keyword != ? AND ? LIKE "%" || window_title_keyword || "%"',
            (sname, '', title)
        ).fetchall()
        if rows:
            return rows[0]['scene']
