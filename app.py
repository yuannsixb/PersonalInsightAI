...
with st.sidebar:
    ...
    if ai_key:
        ...
        st.warning('AI 未配置')
    if st.button('设置', use_container_width=True, type='secondary' if st.session_state.page!='settings' else 'primary'):
...
if st.session_state.page == 'knowledge':
    ...
    from database.db import get_user_rules
    from analysis.user_dictionary import add_rule as ud_add_rule, update_rule as ud_upd_rule, delete_rule as ud_del_rule
        ...
        with st.container(border=True):
            ...
            with st.form('add_rule_form', clear_on_submit=True):
                ...
                if st.form_submit_button('保存规则', use_container_width=True) and new_app:
                    ud_add_rule(new_app.lower(), new_scene, new_kw.strip())
                    refresh_data()
                    st.rerun()
    ...
    if rules:
        ...
        for r in rules:
            ...
            if cols[4].button('删', key=f'dr_{r["id"]}'):
                ud_del_rule(r['id']); refresh_data(); st.rerun()
        if 'editing_rule' in st.session_state:
            ...
            with st.container(border=True):
                ...
                if c1.button('保存修改', use_container_width=True):
                    ud_upd_rule(r['id'], ns); refresh_data()
...
if st.session_state.page == 'settings':
    ...
    with st.container(border=True):
        st.write('**AI API 配置**')
        st.caption('修改后点击保存，所有 AI 功能即刻生效')
        _env_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.env')
        _current_key = os.getenv('AI_API_KEY', '')
        _current_url = os.getenv('AI_BASE_URL', 'https://api.deepseek.com')
        _current_model = os.getenv('AI_MODEL', 'deepseek-v4-flash')

        with st.form('ai_settings_form', clear_on_submit=False):
            k = st.text_input('API Key', type='password', value=_current_key, key='setting_apikey',
                             placeholder='sk-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
            u = st.text_input('API 地址', value=_current_url, key='setting_apiurl')
            m = st.text_input('模型', value=_current_model, key='setting_apimodel',
                             placeholder='deepseek-v4-flash, deepseek-v4-pro, gpt-4o 等')

            col1, col2 = st.columns([1, 3])
            if col1.form_submit_button('保存', use_container_width=True) and k:
                with open(_env_path, 'w') as f:
                    f.write(f'AI_API_KEY={k}\nAI_BASE_URL={u}\nAI_MODEL={m}\n')
                os.environ['AI_API_KEY'] = k
                os.environ['AI_BASE_URL'] = u
                os.environ['AI_MODEL'] = m
                st.success('已保存并生效')
                st.rerun()
            if col2.form_submit_button('清除配置', use_container_width=True):
                if os.path.exists(_env_path):
                    os.remove(_env_path)
                for env in ['AI_API_KEY', 'AI_BASE_URL', 'AI_MODEL']:
                    os.environ.pop(env, None)
                st.rerun()

    if os.getenv('AI_API_KEY'):
        _masked = os.getenv('AI_API_KEY', '')[:6] + '****' + os.getenv('AI_API_KEY', '')[-4:]
        st.success(f'当前状态: ✅ 已配置（{_masked}）')
    else:
        st.warning('当前状态: ❌ 未配置')
    ...
    with st.container(border=True):
        st.write('**获取 API Key**')
        st.markdown('''
- [DeepSeek 官方](https://platform.deepseek.com/) - 注册即送额度
- [阿里云百炼](https://bailian.console.aliyun.com/) - 有免费额度
- [硅基流动](https://cloud.siliconflow.cn/) - 注册送额度
        ''')
...
if not os.getenv('AI_API_KEY'):
    st.warning('API Key 未配置，请前往「设置」页面配置')
else:
